...
Instead of controlling the method of activity aggregation by the user, by default, median (and 95% confidence interval) is applied to summarize the data, including  POD, EC50, Emax, wAUC, and responses. 

```{r sim_out,  dependson = "cal"}
sum_out <- extract_curvep_data(outd, "summary")
```
...
We should check the curves of threshold vs pooled_variance results and threshold vs curvature results. In the plot of threshold vs pooled variance, the ideal curve should look like *percent_affected_96*. At threshold = 25%, the pooled_variance is stabilized. However, for the endpoint, *percent_mortality_96*, there are not enough response variations in the dataset (i.e., almost all chemicals are **clear inactive**), therefore, we cannot derive the baseline noise threshold for this endpoint in this dataset. 
Similarly, in the plot of threshold vs curvature, threshold = 25% is also the largest positive curvature after the lowest negative curvature. 
