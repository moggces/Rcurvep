...
# calculation (not run)
outd <- run_curvep_job(zfishdev_complete, directionality = 1, n_sample = 1000, threshold = seq(5, 95, by = 5), other_paras = list(CARR = 20, TrustHi = TRUE))
...
Activity results are put into the function `identify_basenoise_threshold()`.
...
We should check the threshold vs pooled_variance results and threshold vs curvature results. The ideal curve will look like *percent_affected_96*. However, for the endpoint, *percent_mortality_96*, there are not enough response variations in the dataset (i.e., almost all chemicals are **clear inactive**), therefore, we cannot derive the baseline noise threshold for this endpoint in this dataset using `identify_basenoise_threshold()`. 
...
# curvature
p <- ggplot(outthres, aes(x = thres, y = curvature))
p + geom_point() + geom_line() + facet_wrap(~ endpoint)

```

The thresholds identified are:
