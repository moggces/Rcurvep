...
CARR_cutoff <- 50
outd <- reparam_curvep_job(outd, other_paras = list(CARR = CARR_cutoff, TrustHi = TRUE))
...
acts <- run_curvep_job(zfishbeh, directionality = 1, n_sample = n_sample_test, threshold = threshold_range, other_paras = list(CARR = 20, TrustHi = TRUE), simplify_output = TRUE)
...
# can also extract summarized activity but making any "INVERSE" monotonic curves as inactive
#act_sum <- extract_curvep_data(outd, "summary", modifier = "INVERSE")
...
acts <- run_curvep_job(zfishdev_complete, 
...
We could plot the diagnostic plots by applying the `plot = TRUE`. 


```{r plot_thres, fig.align = "center", fig.width = 6}

outthres <- identify_basenoise_threshold(acts,  plot = TRUE)
# or use generate_diagnostic_plot(outthres) directly

# you can also calculate the threshold based on your own data
# cal_exponential_inflection(filter(outthres, endpoint == "percent_affected_96"), xvar = "threshold", yvar = "pooled_variance")

```

In the plot of threshold vs pooled variance, the ideal curve should look like *percent_affected_96*. At threshold = 25%, the pooled_variance is stabilized. However, for the endpoint, *percent_mortality_96*, there are not enough response variations in the dataset (i.e., almost all chemicals are **clear inactive**), therefore, we cannot derive the baseline noise threshold for this endpoint in this dataset. 

The identified baseline noise thresholds using either the raw pooled variance data or results from the exponential fitting (*thresDist_raw* and *thresDist_exp* columns). The comment for threshold based on the results from the exponential fitting and linear fitting can be found in the *thresComment*, *cor_exp_fit*, and *cor_lm_fit* columns.
