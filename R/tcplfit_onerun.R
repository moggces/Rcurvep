...
#' @param thr_resp The response cutoff to calculate the potency. Default = 20 (POD20)
#' @param perc_resp The percentage cutoff to calculate the potency. Default = 10 (EC10).
...
extract_fit_activity <- function(inp, out, thr_resp, perc_resp) {
  ...
  if (win_modl$modl == "cnst") {
    ...
    if (!is.na(Emax)) {
      ...
      # add the background to bmr
      thr_resp <- thr_resp + win_modl$bt
