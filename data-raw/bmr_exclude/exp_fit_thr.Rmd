...
### temporary test the data from other notebook

```{r}
#test_boot_100 
test_out <- map_df(test_boot_100, ~ .x[[1]], .id = "id") %>% 
  select(id, endpoint, direction, threshold, pooled_variance) %>%
  nest(-endpoint, -direction, -id) %>%
 mutate(
   knee_out = map(data, function(x) cal_knee_point(x, "threshold", "pooled_variance"))
 ) %>%
 select(-data)
