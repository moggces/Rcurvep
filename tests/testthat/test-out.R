...
test_that("remove comment activities", {
  ...
  outd <- run_curvep_batch(x[[1]],
                         ...
                         threshold = 5,
                         other_paras = list(CARR = 20, TrustHi = TRUE))
  acts <- withdraw(outd, "act", modifier = "CHECK")
...
test_that("summarize curvep act data", {
  ...
  acts <- summary(outd, col_names = c("POD", "EC50"), modifier = "INVERSE", conf_level = 0.9)
