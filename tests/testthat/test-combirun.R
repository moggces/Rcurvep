...
test_that("n_samples = NULL", {
  ...
  outp <- combi_run_rcurvep(dat, n_samples = NULL,
                            keep_sets = c("act_set", "resp_set"), TRSH = thresholds, RNGE = ranges)
...
test_that("n_samples != NULL", {
  ...
  thresholds <- c(5, 10)
  n_samples <- 2
  outp <- combi_run_rcurvep(zfishbeh, n_samples = n_samples, keep_sets = c("act_set"), TRSH = thresholds)
  expect_equal(
    nrow(outp$result$act_set),
...
test_that("on a single rcurvep object", {
  ...
  obj <- merge_rcurvep_output(outp, keep_sets = "act_set")
  expect_equal(
